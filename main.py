...
from kivy.graphics import Color, Line, Rectangle, Ellipse, Triangle, Point, Quad
...
class MainWidget(RelativeLayout):
    from transforms import transform, transform_2D, tranform_perspective
    ...
    def update_tiles(self):
        for i in range(0, self.NB_TILES):
            tile = self.tiles[i]
            tile_coordiantes = self.tiles_coordinates[i]
            xmin, ymin = self.get_tile_coordinates(
                tile_coordiantes[0], tile_coordiantes[1]
            )
            xmax, ymax = self.get_tile_coordinates(
                tile_coordiantes[0] + 1, tile_coordiantes[1] + 1
            )
